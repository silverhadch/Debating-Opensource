...
$id = $_SESSION['id'];
$stmt_user = $connection_pdo->prepare("SELECT * FROM users WHERE id = :id");
$stmt_user->execute(['id' => $id]);
$result_user = $stmt_user->fetch(PDO::FETCH_ASSOC);
$res_username = $result_user['username'];
//$res_email = $result_user['email'];
$res_id = $result_user['id'];

// Fetch player results
$stmt_players = $connection_pdo->prepare("SELECT players.*, teams.team_name FROM players INNER JOIN teams ON players.TeamID = teams.Team_ID WHERE team_name LIKE '%Sr.%' ORDER BY player_Max_points DESC");
$stmt_players->execute();
$results = $stmt_players->fetchAll(PDO::FETCH_ASSOC);
?>
